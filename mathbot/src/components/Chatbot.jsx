...
const Chatbot = () => {
    ...
    const handleOptionClick = (option) => {
        if (!isLatihan) {
            ...
            if (isLatihan) {
                setShowNextButton(true); // Show "Next" button in latihan mode
            } else {
                moveToNextStep(option);
            }
        }else{
    ...
    const handleNextClick = () => {
        ...
        moveToNextStep(currentSteps[step].options.find((opt) => opt.key === selectedOption));
    };
                                    ...
                                    <div className="flex flex-col mt-2 ml-4">
                                        {currentSteps[step].options.map((option, index) => (
                                            <button
                                                key={index}
                                                onClick={() => handleOptionClick(option)}
                                                className={`px-4 py-2 rounded-lg mt-2 text-left ${
                                                    selectedOption === option.key ? "bg-green-300" : "bg-gray-300"
                                                }`}
                                            >
                                                {typeof option.text === 'string' ? (
                                                    option.text
